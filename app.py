...
def on_join(data):
    ...
    add_log(f"השחקן {name} נכנס למבוך")
    sync_all()
...
def handle_action(data):
    ...
    if data['type'] == 'flashlight':
        if "flashlight" in p['items'] and "batteries" in p['items']:
            add_log(f"{p['n']} הפעיל פנס {d_name}")
            tx, ty = p['x'], p['y']
            while True:
        ...
        if blocked:
            add_log(f"{p['n']} נתקע בקיר כשניסה לזוז {d_name}")
        else:
            p['x'] += dx; p['y'] += dy
            pos = [p['x'], p['y']]
