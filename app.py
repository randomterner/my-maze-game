...
def handle_move(data):
    ...
    if 0 <= nx < 10 and 0 <= ny < 10:
        ...
        if tile == "river":
            ...
            elif "raft" in p['items']: p['x'], p['y'] = river_start_pos; add_log("LOG_RIVER_RAFT", p['n'])
            else: p['x'], p['y'] = river_start_pos; p['injuries'] += 1; add_log("LOG_RIVER_SWEEP", p['n'])
        elif tile == "armory": p['bul'] = max(p['bul'], 3); p['bom'] = max(p['bom'], 3); add_log("LOG_ARMORY", p['n'])
        elif tile == "monster": p['bul'] = min(5, p['bul'] + 1); p['bom'] = min(5, p['bom'] + 1); add_log("LOG_MONSTER", p['n'])
        elif tile == "devil": p['injuries'] += 1; p['bul'] = max(0, p['bul']-1); p['bom'] = max(0, p['bom']-1); add_log("LOG_DEVIL", p['n'])
        elif tile == "black_hole": p['x'], p['y'] = random.randint(0,9), random.randint(0,9); add_log("LOG_BLACK_HOLE", p['n'])
        elif tile == "clinc" and p['injuries'] < 4: p['injuries'] = 0; add_log("LOG_CLINIC", p['n'])
        elif tile == "er" and p['injuries'] == 4: p['injuries'] = 3; add_log("LOG_ER", p['n'])
        elif tile == "exit" and "treasure" in p['items']: winner = p['n']; add_log("LOG_WIN_EXIT", p['n'])
