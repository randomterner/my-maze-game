...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'maze_god_mode_2026'
socketio = SocketIO(app, cors_allowed_origins="*", async_mode='eventlet')

# 10x10 Grid
# visited_by: שמות שחקנים שעברו כאן
# ex_walls: היסטוריית פיצוצים
maze = [[{
    "tile": "empty", "was": None, 
...
def on_join(data):
    ...
    players[request.sid] = {
        ...
        "is_lost": False, "knows_river_start": False
    }
    ...
    if not blocked:
        ...
        if target_w_cell['ex_walls'][wall_key]['broken']:
            ...
            if broken_total == 1 and (not p['is_lost'] or p['knows_river_start']):
                ...
                if b_obj: add_log(f"🕵️ {p['n']} עבר בקיר השבור היחיד. {breaker_n} נמצא ב-{get_rel_dir(p, b_obj)}.")
        ...
        if t_name and t_name not in ["empty", "river"]:
            ...
            for other in [pl for pl in players.values() if not pl['is_man'] and pl['n'] in tile['visited_by'] and pl['n'] != p['n']]:
                # שיתוף מידע: אם אבוד, משתף רק מה שגילה מאז שהלך לאיבוד
                to_share = p['post_lost_tiles'] if p['is_lost'] else p['known_tiles']
                for t_coord in to_share:
                    if t_coord not in other['known_tiles']: other['known_tiles'].append(t_coord)
                if not p['is_lost']: # אם לא אבוד, מקבל גם מידע מהשני
                    for t_coord in other['known_tiles']:
                        if t_coord not in p['known_tiles']: p['known_tiles'].append(t_coord)
                add_log(f"🤝 מפות של {p['n']} ו-{other['n']} אוחדו דרך ה-{t_name}!")

        # אפקטים של כל המשבצות
        if tile['tile'] == "river":
            ...
            else: add_log(f"🌊 {p['n']} נסחף (ללא פציעה).")
