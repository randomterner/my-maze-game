...
def set_phase(data):
    ...
    try:
        ...
        if new_p > game_phase: # Forward only
            game_phase = new_p
            sync_all()
    except: pass
...
def handle_action(data):
    ...
    if act == 'shoot' and p['bul'] > 0:
        ...
        while 0 <= tx <= 9 and 0 <= ty <= 9:
            ...
            if target: target['injuries'] += 1; add_log(f"Hit! {target['n']} injured"); break
        ...
        if not blocked:
            ...
            if tile == "monster": 
                p['bul']+=1; p['bom']+=1; sync_all(); return # Double turn
            elif tile == "devil": 
