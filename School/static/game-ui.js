(() => {
  ...
  window.createMapView = (board, large = false) => {
    ...
    const square = document.createElement('div'); square.className = 'mapSquare'; wrap.appendChild(square);
    const grid = document.createElement('div'); grid.className = 'mapGrid'; square.appendChild(grid);
    const players = board.players || [];
    const points = [...Object.keys(board.tiles || {}).map(key => key.split(',').map(Number)), ...players.map(p => [p.x,p.y]),
      ...[...(board.open_edges||[]),...(board.wall_edges||[]),...(board.broken_walls||[])].flat()];
    ...
    if(board.manager_map&&large){
      ...
      wrap.replaceChild(axes,square);axes.append(cols,rows,square);
    }
    return wrap;
  };
